...
class RenderingCanvas(app.Canvas):
	...
	def __init__(self,
		...
		print(random.uniform(0.1, 600000000000.2))
		self.program['iGlobalTime'] = random.uniform(0.1, 600000000000.2)/10000

		self.program['iOffset'] = 0.0, 0.0

		self.activate_zoom()
		if channel0 is not None:
			self.set_channel_input(Image.open(channel0), i=0)
		if channel1 is not None:
			self.set_channel_input(Image.open(channel1), i=1)
		if channel2 is not None:
			self.set_channel_input(Image.open(channel2), i=2)
		if channel3 is not None:
			self.set_channel_input(Image.open(channel3), i=3)
	...
	def advance_time(self):
		if not self._paused:
			if self._interval == 'auto':
				print('mdr')
				self.program['iGlobalTime'] = time.time()#time.clock() - self._clock_time_zero
			else:
				print('saywhot')
...
if __name__ == '__main__':
	...
	parser.add_argument('--channel0', type=str, help="use the image file as uniform in channel0.")
	parser.add_argument('--channel1', type=str, help="use the image file as uniform in channel1.")
	parser.add_argument('--channel2', type=str, help="use the image file as uniform in channel2.")
	parser.add_argument('--channel3', type=str, help="use the image file as uniform in channel3.")
